...
@section('container')
    <div class="w-full h-max bg-white p-4 rounded-lg mb-36">
        <div class="text-xl font-medium mb-4">Keranjang</div>
        <form action="/note" method="post" class="flex flex-col gap-3 mb-4">
            @method('post')
            @csrf
            @php
            ...
            @foreach ($carts as $index => $cart)
                ...
                <div class="bg-white w-full h-max border rounded-lg shadow-md flex flex-row gap-3 px-3 py-6 mt-3">
                    ...
                    <div>
                        <img src="{{ asset('storage/' . $cart->product->gambar) }}" alt="" class="w-20 h-20 lg:w-32 lg:h-32 object-cover rounded-lg">
                    </div>
                    <div class="lg:w-full">
            ...
            <button class="bg-green-600 w-full text-white mt-4 px-3 py-2 rounded-lg hover:bg-green-500 hover:duration-150 lg:w-max lg:px-8">Transaksi</button>
                @endif
        </form>
    </div>
    <script>
